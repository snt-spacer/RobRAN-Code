...
class FloatingPlatformRobot(RobotCore):
    def __init__(self, robot_cfg: FloatingPlatformRobotCfg, robot_uid: int = 0, num_envs: int = 1, device: str = "cuda"):
        ...
        self._thrust_action = torch.zeros((num_envs, self._robot_cfg.num_thrusters), device=device, dtype=torch.float32)
        if self._robot_cfg.is_reaction_wheel:
            self._reaction_wheel_action = torch.zeros((num_envs, 1), device=device, dtype=torch.float32)


    def run_setup(self, articulation: Articulation):
        self._thrusters_dof_idx, _ = articulation.find_bodies(self._robot_cfg.thrusters_dof_name)
        
        if self._robot_cfg.is_reaction_wheel:
            self._reaction_wheel_dof_idx, _ = articulation.find_joints(self._robot_cfg.reaction_wheel_dof_name)

    def create_logs(self):
        super(FloatingPlatformRobot, self).create_logs()
    ...
    def get_observations(self, robot_data: ArticulationData) -> torch.Tensor:
        ...
        print(f"Robot data positions: {robot_data.root_pos_w}")
    ...
    def set_initial_conditions(self, env_ids: torch.Tensor, articulations: Articulation):
        ...
        articulations.set_external_force_and_torque(thrust_reset, thrust_reset, body_ids=self._thrusters_dof_idx, env_ids=env_ids)
        if self._robot_cfg.is_reaction_wheel:
            rw_reset = torch.zeros_like(self._reaction_wheel_action)
            articulations.set_joint_velocity_target(rw_reset, joint_ids=self._reaction_wheel_dof_idx, env_ids=env_ids)
            articulations.set_joint_effort_target(rw_reset, joint_ids=self._reaction_wheel_dof_idx, env_ids=env_ids)
