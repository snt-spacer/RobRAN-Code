...
from omni.isaac.lab.utils.assets import ISAAC_NUCLEUS_DIR, get_local_path

from omni.isaac.lab_tasks.rans import GoToPositionCfg, GoToPositionTask, JackalRobot, JackalRobotCfg


@configclass
class JackalGoToPositionEnvCfg(DirectRLEnvCfg):
    # env
    decimation = 4
    ...
    robot_cfg: JackalRobotCfg = JackalRobotCfg()
    task_cfg: GoToPositionCfg = GoToPositionCfg()
    debug_vis: bool = True


class JackalGoToPositionEnv(DirectRLEnv):
    # Workflow: Step
    ...
    def _setup_scene(self):
        ...
        self.robot_api = JackalRobot(self.cfg.robot_cfg, robot_uid=0, num_envs=self.num_envs, device=self.device)
        ...
        self.scene.articulations["jackal"] = self.robot
        # add lights
